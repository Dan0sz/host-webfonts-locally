...
class OMGF {
	...
	public static function get( $name, $default = null ) {
		...
		if ( ! $default && $name === OMGF_Admin_Settings::OMGF_ADV_SETTING_SUBSETS ) {
	...
	public static function task_manager_warnings() {
		if ( ! empty( self::get( OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_TEST_MODE ) ) && ! wp_doing_ajax() ) : ?>
							...
							<?php foreach ( $warnings as $warning_id ) : ?>
								...
								<li id="omgf-notice-<?php echo $warning_id; ?>">
									...
									<?php if ( in_array( str_replace( '-req-pro', '', $warning_id ), self::THEMES_REQ_PRO ) ) : ?>
										...
										<?php echo sprintf( __( 'Due to the exotic way your theme (%1$s) implements Google Fonts, OMGF Pro\'s Advanced Processing features are required to detect them. <a href="%2$s" target="_blank">Upgrade and install OMGF Pro</a> to continue.', 'host-webfonts-local' ), ucfirst( str_replace( '-req-pro', '', $warning_id ) ), OMGF_Admin_Settings::FFWP_WORDPRESS_PLUGINS_OMGF_PRO ); ?>
									<?php endif; ?>
									<?php if ( in_array( str_replace( '-addtnl-conf', '', $warning_id ), self::THEMES_ADDTNL_CONF ) ) : ?>
										...
										<?php echo sprintf( __( 'Your theme (%1$s) requires additional configuration to be compatible with %2$s, check the list of <a href="%3$s" target="_blank">known issues</a> to fix it.', 'host-webfonts-local' ), ucfirst( $template_id ), apply_filters( 'omgf_settings_page_title', 'OMGF' ), OMGF_Admin_Settings::DAAN_DOCS_OMGF_PRO_KNOWN_ISSUES ); ?>
									<?php endif; ?>
									...
									<?php if ( in_array( str_replace( '-req-pro', '', $warning_id ), self::PLUGINS_REQ_PRO ) ) : ?>
										...
										<?php echo sprintf( __( 'Due to the exotic way the plugin, <strong>%1$s</strong>, implements Google Fonts, OMGF Pro\'s Advanced Processing features are required to detect them. <a href="%2$s" target="_blank">Upgrade and install OMGF Pro</a> to continue.', 'host-webfonts-local' ), $plugin_name, OMGF_Admin_Settings::FFWP_WORDPRESS_PLUGINS_OMGF_PRO ); ?>
									<?php endif; ?>
									<?php if ( in_array( str_replace( '-addtnl-conf', '', $warning_id ), self::PLUGINS_ADDTNL_CONF ) ) : ?>
										...
										<?php echo sprintf( __( 'The plugin, <strong>%1$s</strong>, requires additional configuration to be compatible with %2$s. Check the <a href="%3$s" target="_blank">list of known issues</a> to fix it.', 'host-webfonts-local' ), $plugin_name, apply_filters( 'omgf_settings_page_title', 'OMGF' ), OMGF_Admin_Settings::DAAN_DOCS_OMGF_PRO_KNOWN_ISSUES ); ?>
									<?php endif; ?>
	...
	public static function debug( $message ) {
		if (
			! self::get( OMGF_Admin_Settings::OMGF_ADV_SETTING_DEBUG_MODE ) ||
			( self::get( OMGF_Admin_Settings::OMGF_ADV_SETTING_DEBUG_MODE ) && file_exists( self::$log_file ) && filesize( self::$log_file ) > MB_IN_BYTES )
		) {
			return;
		}
