...
class OMGF_AJAX
{
	...
	public function __construct () {
		...
		add_action( 'wp_ajax_omgf_ajax_search_font_subsets', [ $this, 'search_font_subsets' ] );
		add_action( 'wp_ajax_omgf_ajax_search_google_fonts', [ $this, 'search_fonts' ] );
		add_action( 'wp_ajax_omgf_ajax_process_font_styles_queue', [ $this, 'process_font_styles_queue' ] );
		add_action( 'wp_ajax_omgf_ajax_download_fonts', [ $this, 'download_fonts' ] );
		add_action( 'wp_ajax_omgf_ajax_generate_styles', [ $this, 'generate_styles' ] );
		...
		foreach ( $searchQueries as $searchQuery ) {
			...
			if ( empty( $subsets ) ) {
				OMGF_Admin_Notice::set_notice(
					sprintf( __( 'Font %s not found. Are you sure it\'s a Google Font?', $this->plugin_text_domain ), $searchQuery ),
					false,
					'error'
				);
	...
	public function search_fonts () {
		...
		OMGF_Admin_Notice::set_notice(
			count( $fonts ) . ' ' . __( 'font styles found. Trim the list to your needs and click <strong>Download Fonts</strong>.', $this->plugin_text_domain )
		);
	}
	...
	public function process_font_styles_queue () {
		...
		$selected_fonts = array_filter(
			$current_fonts,
			function ( $font_style ) use ( $selected_fonts ) {
		...
		OMGF_Admin_Notice::set_notice(
			count( $current_fonts ) - count( $selected_fonts ) . ' ' . __( 'fonts removed from list and', $this->plugin_text_domain ) . ' ' . count(
				$to_be_preloaded
			) . ' ' . __(
				'fonts set to preload. If you haven\'t already, you can now <strong>download</strong> the <strong>fonts</strong>. Otherwise, just (re-)<strong>generate</strong> the <strong>stylesheet</strong>.',
