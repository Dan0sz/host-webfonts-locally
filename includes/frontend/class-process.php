...
class OMGF_Frontend_Process {
	...
	private function init() {
		/**
		  ...
		  */
		$test_mode_enabled = ! empty( OMGF::get( OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_TEST_MODE ) );
	...
	public function build_fonts_set( $links, $handle = 'omgf-stylesheet' ) {
		...
		foreach ( $links as $key => $link ) {
			 ...
			 */
			if ( OMGF::get( OMGF_Admin_Settings::OMGF_ADV_SETTING_COMPATIBILITY ) && strpos( $id, 'et-builder-googlefonts' ) !== false ) {
				$google_fonts[ $key ]['id'] = $id . '-' . strlen( $href['href'] );
			} elseif ( OMGF::get( OMGF_Admin_Settings::OMGF_ADV_SETTING_COMPATIBILITY ) && $id === 'google-fonts-1' ) {
				/**
