...
class OMGF_Admin_Settings_Optimize extends OMGF_Admin_Settings_Builder {
		...
		?>
			...
			<tr valign="top">
				...
				<td class="task-manager-row">
					...
					<?php else : ?>
						<p>
							<?php echo __( 'No stylesheets found. <a href="#" id="omgf-save-optimize">Start optimization</a>?', 'host-webfonts-local' ); ?> <?php echo get_option( OMGF_Admin_Settings::OMGF_OPTIMIZE_HAS_RUN ) ? sprintf( __( '(If optimization seems to be failing, read <a href="%s" target="_blank">this</a>.)', 'host-webfonts-local' ), 'https://daan.dev/docs/omgf-pro-troubleshooting/no-fonts-detected/' ) : ''; ?>
						</p>
					<?php endif; ?>
				</td>
			</tr>
			...
			$this->do_checkbox(
				...
				! empty( OMGF::get( OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_AUTO_SUBSETS ) ),
			...
			$this->do_checkbox(
				...
				! empty( OMGF::get( 'omgf_pro_auto_config' ) ),
			...
			<tr>
				...
				<td class="task-manager-row">
					<a id="omgf-empty" data-init="<?php echo OMGF_Admin_Settings::OMGF_ADMIN_PAGE; ?>" data-nonce="<?php echo wp_create_nonce( OMGF_Admin_Settings::OMGF_ADMIN_PAGE ); ?>" class="omgf-empty button-cancel"><?php _e( 'Empty Cache Directory', 'host-webfonts-local' ); ?></a>
					<a id="omgf-refresh" data-init="<?php echo OMGF_Admin_Settings::OMGF_ADMIN_PAGE; ?>" data-nonce="<?php echo wp_create_nonce( OMGF_Admin_Settings::OMGF_ADMIN_PAGE ); ?>" class="omgf-refresh button-cancel"><?php _e( 'Refresh Cache (and maintain settings)', 'host-webfonts-local' ); ?></a>
				</td>
			</tr>
		<?php
	}
	...
	public function do_display_option() {
		$this->do_select(
			...
			OMGF::get( OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_DISPLAY_OPTION ),
	...
	public function do_promo_apply_font_display_globally() {
		$this->do_checkbox(
			...
			! empty( OMGF::get( 'omgf_pro_force_font_display' ) ),
	...
	public function do_promo_remove_async_google_fonts() {
		$this->do_checkbox(
			...
			! empty( OMGF::get( 'omgf_pro_remove_async_fonts' ) ),
	...
	private function do_optimized_fonts_manager() {         
		?>
			<div class="omgf-optimize-fonts-manage">
				...
				<div class="omgf-optimize-fonts-tooltip">
					<ul>
						<li class="dashicons-before dashicons-info-outline">
							<em><?php echo sprintf( __( 'This list is populated with all Google Fonts stylesheets captured and downloaded throughout your site. It will grow organically if other Google Fonts stylesheets are discovered throughout your site.', 'host-webfonts-local' ), get_site_url() ); ?></em>
						</li>
					</ul>
				</div>
				<table>
						...
						<tbody class="stylesheet" id="<?php echo $handle; ?>">
							...
							<?php foreach ( $fonts as $font ) : ?>
								...
								if ( ! is_object( $font ) || count( (array) $font->variants ) <= 0 ) {
									continue;} 
								?>
								<tr class="font-family" data-id="<?php echo $handle . '-' . $font->id; ?>">
									<td colspan="5">
										<span class="family"><em><?php echo rawurldecode( $font->family ); ?></em></span> <span class="unload-mass-action">(<a class="unload-italics"><?php echo __( 'Unload italics', 'host-webfonts-local' ); ?></a> <span class="dashicons dashicons-info tooltip"><span class="tooltip-text"><?php echo __( 'In most situations you can safely unload all Italic font styles. Modern browsers are capable of mimicking Italic font styles.', 'host-webfonts-local' ); ?></span></span> | <a class="unload-all"><?php echo __( 'Unload all', 'host-webfonts-local' ); ?></a> | <a class="load-all"><?php echo __( 'Load all', 'host-webfonts-local' ); ?></a>)</span>
									</td>
									<td class="fallback-font-stack">
										<select data-handle="<?php echo $handle; ?>" <?php echo ! defined( 'OMGF_PRO_FALLBACK_FONT_STACK' ) ? 'disabled' : ''; ?> name="omgf_pro_fallback_font_stack[<?php echo $handle; ?>][<?php echo $font->id; ?>]">
											<option value=''><?php echo __( 'None (default)', 'host-webfonts-local' ); ?></option>
											<?php foreach ( OMGF_Admin_Settings::OMGF_FALLBACK_FONT_STACKS_OPTIONS as $value => $label ) : ?>
												<option <?php echo defined( 'OMGF_PRO_FALLBACK_FONT_STACK' ) && isset( OMGF_PRO_FALLBACK_FONT_STACK[ $handle ][ $font->id ] ) && OMGF_PRO_FALLBACK_FONT_STACK[ $handle ][ $font->id ] == $value ? 'selected' : ''; ?> value="<?php echo $value; ?>"><?php echo $label; ?></option>
											<?php endforeach; ?>
										</select>
									</td>
									<td class="replace">
										...
										?>
										<input autocomplete="off" type="checkbox" class="replace" <?php echo $replace; ?> <?php echo $fallback ? '' : 'disabled'; ?> <?php echo ! defined( 'OMGF_PRO_REPLACE_FONT' ) ? 'disabled' : ''; ?> name="omgf_pro_replace_font[<?php echo $handle; ?>][<?php echo $font->id; ?>]" />
									</td>
								</tr>
								<?php $id = ''; ?>
								<?php foreach ( $font->variants as $variant ) : ?>
									...
									<tr>
										...
										<td class="preload-<?php echo $class; ?>">
											<input data-handle="<?php echo $handle; ?>" data-font-id="<?php echo $handle . '-' . $font->id; ?>" autocomplete="off" type="checkbox" class="preload" name="<?php echo OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_PRELOAD_FONTS; ?>[<?php echo $handle; ?>][<?php echo $font->id; ?>][<?php echo $variant->id; ?>]" value="<?php echo $variant->id; ?>" <?php echo $preload ? 'checked="checked"' : ''; ?> <?php echo $unload ? 'disabled' : ''; ?> />
										</td>
										<td class="unload-<?php echo $class; ?>">
											<input data-handle="<?php echo $handle; ?>" data-font-id="<?php echo $handle . '-' . $font->id; ?>" autocomplete="off" type="checkbox" class="unload" name="<?php echo OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_UNLOAD_FONTS; ?>[<?php echo $handle; ?>][<?php echo $font->id; ?>][<?php echo $variant->id; ?>]" value="<?php echo $variant->id; ?>" <?php echo $unload ? 'checked="checked"' : ''; ?> <?php echo $preload ? 'disabled' : ''; ?> />
										</td>
									</tr>
								<?php endforeach; ?>
							<?php endforeach; ?>
				...
				<input type="hidden" name="<?php echo OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_OPTIMIZED_FONTS; ?>" value="<?php echo base64_encode( serialize( $this->optimized_fonts ) ); ?>" />
				<input id="<?php echo OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_UNLOAD_STYLESHEETS; ?>" type="hidden" name="<?php echo OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_UNLOAD_STYLESHEETS; ?>" value="<?php echo esc_attr( implode( ',', OMGF::unloaded_stylesheets() ) ); ?>" />
				<input id="<?php echo OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_CACHE_KEYS; ?>" type="hidden" name="<?php echo OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_CACHE_KEYS; ?>" value="<?php echo esc_attr( implode( ',', $cache_handles ) ); ?>" />
	...
	public function do_test_mode() {
		$this->do_checkbox(
			...
			! empty( OMGF::get( OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_TEST_MODE ) ),
