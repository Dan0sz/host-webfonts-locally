...
class OMGF_API_Download extends WP_REST_Controller
{
	...
	public function process ( $request ) {
		...
		$wanted_subsets = $params['subset'] ? explode( ',', $params['subset'] ) : [ 'latin', 'latin-ext' ];
		$stylesheet     = $this->filter_subsets( $response['body'], $wanted_subsets );
		$stylesheet     = "/** Generated by OMGF Pro */\n" . implode( "\n", $stylesheet );
	...
	/**
	 * Due to a bug in the Google Fonts API, we're retrieving all the subsets when a user agent is specified. That's why we're filtering them here.
	 */
	private function filter_subsets ( $stylesheet, $wanted ) {
		preg_match_all( '/\/\*.*?\*\/.*?\}/s', $stylesheet, $font_subsets );
		
		$font_subsets = reset( $font_subsets );
		
		return array_filter(
			$font_subsets,
			function ( $value ) use ( $wanted ) {
				foreach ( $wanted as $subset ) {
					if ( strpos( $value, ' ' . $subset . ' ' ) !== false ) {
						return true;
					}
				}
