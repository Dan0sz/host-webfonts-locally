...
class OMGF_Optimize_Run {
	...
	private function optimization_succeeded() {
		...
		if ( ! empty( OMGF::get( OMGF_Admin_Settings::OMGF_OPTIMIZE_SETTING_AUTO_SUBSETS ) ) ) {
			/**
			 ...
			 */
			if ( $available_used_subsets && ! empty( $diff = array_diff( OMGF::get( OMGF_Admin_Settings::OMGF_ADV_SETTING_SUBSETS ), $available_used_subsets ) ) ) {
			...
			if ( ! empty( $diff = array_diff( OMGF::get( OMGF_Admin_Settings::OMGF_ADV_SETTING_SUBSETS ), [ 'latin' ] ) ) ) {
