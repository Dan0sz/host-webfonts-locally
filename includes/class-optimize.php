...
class OMGF_Optimize {
	...
	public function process() {
		if ( ! $this->handle || ! $this->original_handle ) {
			OMGF_Admin_Notice::set_notice( sprintf( __( 'OMGF couldn\'t find required stylesheet handle parameter while attempting to talk to API. Values sent were <code>%1$s</code> and <code>%2$s</code>.', 'host-webfonts-local' ), $this->original_handle, $this->handle ), 'omgf-api-handle-not-found', 'error', 406 );
		 ...
		 */
		if ( ! empty( OMGF::get( OMGF_Admin_settings::OMGF_ADV_SETTING_SUBSETS ) ) ) {
	...
	private function parse_variants( $stylesheet, $font_family ) {
		...
		foreach ( $font_faces[0] as $key => $font_face ) {
			 ...
			 */
			if ( ! empty( $subset ) && isset( $subset[1] ) && ! in_array( $subset[1], OMGF::get( OMGF_Admin_settings::OMGF_ADV_SETTING_SUBSETS ) ) && ! is_numeric( $subset[1] ) ) {
				continue;
			}
	...
	private function parse_subsets( $stylesheet, $font_family ) {
		 ...
		 */
		$this->available_used_subsets[ $font_family ] = array_intersect( $subsets, OMGF::get( OMGF_Admin_Settings::OMGF_ADV_SETTING_SUBSETS ) );
