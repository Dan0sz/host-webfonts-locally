...
class OMGF_Admin
{
		...
		if ( $new_cache_dir !== $old_cache_dir && ! empty( $new_cache_dir ) ) {
			...
			if ( ! OMGF_CACHE_URI ) {
				...
				OMGF_Admin_Notice::set_notice(
					__(
						"You've changed OMGF's storage folder to <strong>$new_cache_dir</strong>. Regenerate the stylesheet to implement this change.",
						'host-webfonts-local'
					),
					false,
					'info'
				...
				OMGF_Admin_Notice::set_notice(
					__(
						"You've changed OMGF's storage folder to <strong>$new_cache_dir</strong>. Make sure the setting <em>Serve font files from...</em> reflects your changes and regenerate the stylesheet.",
						'host-webfonts-local'
					),
					false
				);
	...
	public function serve_uri_changed ( $new_uri, $old_uri ) {
		if ( $new_uri !== $old_uri && ! empty( $new_uri ) ) {
			...
			OMGF_Admin_Notice::set_notice(
				__( "Fonts updated successfully. Regenerate the stylesheet to <em>serve font files from</em> $new_uri.", 'host-webfonts-local' ),
				false,
				'info'
			);
		}
		...
		if ( $new_value != $old_value ) {
			...
			if ( $result ) {
				OMGF_Admin_Notice::set_notice(
					sprintf(
						__(
							'You\'ve %s using <em>relative URLs</em>. <strong>Download</strong> the <strong>fonts</strong> again and (re-)<strong>generate</strong> the <strong>stylesheet</strong> to implement this change.',
							'host-webfonts-local'
						),
						$status
					),
			...
			OMGF_Admin_Notice::set_notice(
				__(
					'You\'ve %s using <em>relative URLs</em>. Something went wrong while updating the fonts. <strong>Empty</strong> the <strong>cache directory</strong>, <strong>download</strong> the <strong>fonts</strong> and <strong>generate</strong> the <strong>stylesheet</strong> to implement this change.',
					'host-webfonts-local'
				),
				false,
				'error'
	...
	public function cdn_url_changed ( $new_url, $old_url ) {
		if ( $new_url != $old_url ) {
			...
			if ( $result ) {
				OMGF_Admin_Notice::set_notice(
					__(
						'Fonts updated successfully. <strong>Download</strong> the <strong>fonts</strong> and (re-)<strong>generate</strong> the <strong>stylesheet</strong> to <em>serve fonts from CDN</em>.',
						'host-webfonts-local'
					),
					false,
					'info'
				...
				OMGF_Admin_Notice::set_notice(
					__(
						'Something went wrong while updating your settings. <strong>Empty</strong> the <strong>Cache Directory</strong>, <strong>download</strong> the <strong>fonts</strong> and <strong>generate</strong> the <strong>stylesheet</strong> to <em>serve fonts from CDN</em>.',
						'host-webfonts-local'
					),
					false,
					'error'
	...
	private function move_files ( $files, $destination ) {
		...
		if ( ! empty( $errors ) ) {
			...
			OMGF_Admin_Notice::set_notice(
				$errored_files . __( 'could not be moved. Do it manually and then regenerate the stylesheet.', 'host-webfonts-local' ),
				false,
				'error'
			);
