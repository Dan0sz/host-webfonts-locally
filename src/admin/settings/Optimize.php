		...
		?>
			...
			<tr valign="top">
				...
				<td class="task-manager-row">
					...
					<?php else : ?>
						<p>
							<?php echo __('No stylesheets found. <a href="#" id="omgf-save-optimize">Start optimization</a>?', 'host-webfonts-local'); ?> <?php echo get_option(Settings::OMGF_OPTIMIZE_HAS_RUN) ? sprintf(__('(If optimization seems to be failing, read <a href="%s" target="_blank">this</a>.)', 'host-webfonts-local'), 'https://daan.dev/docs/omgf-pro-troubleshooting/no-fonts-detected/') : ''; ?>
						</p>
					<?php endif; ?>
				</td>
			</tr>
	...
	public function do_optimize_fonts_contents()
	{
		...
		private function do_optimized_fonts_manager()
		{
			...
			<div class="omgf-optimize-fonts-manage">
								...
								<?php foreach ($font->variants as $variant) : ?>
									...
									<tr>
										...
										<td class="preload-<?php echo $class; ?>">
											<input data-handle="<?php echo $handle; ?>" data-font-id="<?php echo $handle . '-' . $font->id; ?>" autocomplete="off" type="checkbox" class="preload" name="<?php echo Settings::OMGF_OPTIMIZE_SETTING_PRELOAD_FONTS; ?>[<?php echo $handle; ?>][<?php echo $font->id; ?>][<?php echo $variant->id; ?>]" value="<?php echo $variant->id; ?>" <?php echo $preload ? 'checked="checked"' : ''; ?> <?php echo $unload ? 'disabled' : ''; ?> />
										</td>
										<td class="unload-<?php echo $class; ?>">
											<input data-handle="<?php echo $handle; ?>" data-font-id="<?php echo $handle . '-' . $font->id; ?>" autocomplete="off" type="checkbox" class="unload" name="<?php echo Settings::OMGF_OPTIMIZE_SETTING_UNLOAD_FONTS; ?>[<?php echo $handle; ?>][<?php echo $font->id; ?>][<?php echo $variant->id; ?>]" value="<?php echo $variant->id; ?>" <?php echo $unload ? 'checked="checked"' : ''; ?> <?php echo $preload ? 'disabled' : ''; ?> />
										</td>
									</tr>
								<?php endforeach; ?>
							<?php endforeach; ?>
				...
				<input id="<?php echo Settings::OMGF_OPTIMIZE_SETTING_UNLOAD_STYLESHEETS; ?>" type="hidden" name="<?php echo Settings::OMGF_OPTIMIZE_SETTING_UNLOAD_STYLESHEETS; ?>" value="<?php echo esc_attr(implode(',', OMGF::unloaded_stylesheets())); ?>" />
				<input id="<?php echo Settings::OMGF_OPTIMIZE_SETTING_CACHE_KEYS; ?>" type="hidden" name="<?php echo Settings::OMGF_OPTIMIZE_SETTING_CACHE_KEYS; ?>" value="<?php echo esc_attr(implode(',', $cache_handles)); ?>" />
