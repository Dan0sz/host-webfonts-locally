...
class Advanced extends Builder {
	...
	public function do_compatibility() {
		$this->do_checkbox(
			...
			! empty( OMGF::get( Settings::OMGF_ADV_SETTING_COMPATIBILITY ) ),
	...
	public function do_used_subsets() {
		$this->do_select(
			...
			OMGF::get( Settings::OMGF_ADV_SETTING_SUBSETS ),
			( ! empty( OMGF::get( Settings::OMGF_OPTIMIZE_SETTING_AUTO_SUBSETS ) ) ? '<span class="used-subsets-notice info">' . sprintf( __( 'Any changes made to this setting will be overwritten, because <strong>Auto-configure Subsets</strong> is enabled. <a href="%s">Disable it</a> if you wish to manage <strong>Used Subset(s)</strong> yourself. <u>Novice users shouldn\'t change this setting</u>!', 'host-webfonts-local' ), admin_url( Settings::OMGF_OPTIONS_GENERAL_PAGE_OPTIMIZE_WEBFONTS ) ) . '</span>' : '' ) . __( 'A subset is a (limited) set of characters belonging to an alphabet. Default: <code>latin</code>, <code>latin-ext</code>. Limit the selection to subsets your site actually uses. Selecting <u>too many</u> subsets can negatively impact performance! <em>Latin Extended and Vietnamese are an add-ons for Latin and can\'t be used by itself. Use CTRL + click to select multiple values.</em>', 'host-webfonts-local' ),
			true
		);
	}
	...
	public function do_debug_mode() {
		$this->do_checkbox(
			...
			! empty( OMGF::get( Settings::OMGF_ADV_SETTING_DEBUG_MODE ) ),
	...
	public function do_download_log() {
		if ( ! empty( OMGF::get( Settings::OMGF_ADV_SETTING_DEBUG_MODE ) ) ) :
			?>
			<tr>
				<th></th>
				<td>
	...
	public function do_uninstall() {
		$this->do_checkbox(
			...
			! empty( OMGF::get( Settings::OMGF_ADV_SETTING_UNINSTALL ) ),
