							...
							<?php foreach ( $warnings as $warning_id ) : ?>
								...
								<li id="omgf-notice-<?php echo $warning_id; ?>">
									<?php if ( $warning_id == 'is_multisite' ) : ?>
										<?php echo sprintf( __( 'It seems like Multisite is enabled. OMGF doesn\'t natively support Multisite. If you\'re getting CORS related errors on any of your network\'s sites, consider <a href="%s" target="_blank">upgrading to OMGF Pro</a>.', 'host-webfonts-local' ), Settings::FFWP_WORDPRESS_PLUGINS_OMGF_PRO ); ?>
									<?php endif; ?>
									...
									<?php if ( in_array( str_replace( '-req-pro', '', $warning_id ), self::THEMES_REQ_PRO ) ) : ?>
										...
										<?php echo sprintf( __( 'Due to the exotic way your theme (%1$s) implements Google Fonts, OMGF Pro\'s Advanced Processing features are required to detect them. <a href="%2$s" target="_blank">Upgrade and install OMGF Pro</a> to continue.', 'host-webfonts-local' ), ucfirst( str_replace( '-req-pro', '', $warning_id ) ), Settings::FFWP_WORDPRESS_PLUGINS_OMGF_PRO ); ?>
									<?php endif; ?>
									...
									<?php if ( in_array( str_replace( '-req-pro', '', $warning_id ), self::PLUGINS_REQ_PRO ) ) : ?>
										...
										<?php echo sprintf( __( 'Due to the exotic way the plugin, <strong>%1$s</strong>, implements Google Fonts, OMGF Pro\'s Advanced Processing features are required to detect them. <a href="%2$s" target="_blank">Upgrade and install OMGF Pro</a> to continue.', 'host-webfonts-local' ), $plugin_name, Settings::FFWP_WORDPRESS_PLUGINS_OMGF_PRO ); ?>
									<?php endif; ?>
